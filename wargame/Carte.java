		...
		if (perso instanceof Heros) {
			...
			for (int i = 0; i < nbHerosVivant; i++) {
				if (armeeHeros[i].equals(perso)) {
					...
					// Si on le trouve on le déplace jusqu'a la fin en décalant les autres de une
					// pos pour garder une liste d'armée héors vivante de 0 à nbHerosVivant
		...
		if (perso instanceof Monstre) {
			...
			for (int i = 0; i < nbMonstreVivant; i++) {
				if (armeeMonstre[i].equals(perso)) {
					...
					// Si on le trouve on le déplace jusqu'a la fin en décalant les autres de une
					// pos pour garder une liste d'armée monstres vivants de 0 à nbMonstreVivant
	...
	public void toutDessiner(Graphics g) {
			...
			for (int y = 0; y < LARGEUR_CARTE; y++) {
				switch (getElement(x, y).getClass().getSimpleName()) {
					case ("Plaine"):
						...
						break;
					case ("Obstacle"):
						g.setColor(COULEUR_ROCHER); // Obstacle: prendre en compte si c'est de l'eau, un rocher, une
													// forêt (dans Obstacle.java)
						g.fillRect(y * NB_PIX_CASE, x * NB_PIX_CASE, NB_PIX_CASE, NB_PIX_CASE);
						break;
					case ("Heros"):
		...
		if (select.getY() != -1) { // Si pas de selecetion x = -1 et y = -1
			if (select.getX() >= 0 && select.getX() < hauteur && select.getY() >= 0 && select.getY() < largeur) {
				g.drawRect(select.getY() * NB_PIX_CASE, select.getX() * NB_PIX_CASE, NB_PIX_CASE, NB_PIX_CASE); // Draw
																												// : Y
																												// puis
																												// X
